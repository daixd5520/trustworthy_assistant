...
In this workspace, prefer the project wrapper below instead of relying on persisted `~/.mmx` credentials:

```bash
python3 .dbg/mmx_from_env.py <mmx-subcommand> [flags]
```

The wrapper:

- reads the MiniMax API key from `.env`
- defaults `MINIMAX_REGION=cn`
- avoids permission issues when `mmx` cannot write to `~/.mmx`

Examples:

```bash
python3 .dbg/mmx_from_env.py quota show --output json --quiet
python3 .dbg/mmx_from_env.py vision describe --image photo.jpg --prompt "Describe the image." --output json --quiet
```
